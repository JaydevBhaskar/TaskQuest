...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/tasks/:id/progress", isAuthenticated, async (req, res) => {
    try {
      ...
      if (task.owner_id !== user.id) {
        const isMember = (await storage.getTaskMembers(taskId)).some(
  ...
  app.post("/api/tasks/:id/members", isAuthenticated, async (req, res) => {
    try {
      ...
      if (members.some((member) => member.user_id === user_id)) {
        return res
          .status(400)
          .json({ message: "User is already a member of this task" });
      }
  ...
  app.get("/api/leaderboard", isAuthenticated, async (req, res) => {
    try {
      const { data: users, error } = await supabase
        .from("users")
        .select(
          "id, username, email, avatar_url, level, xp, streak, tasks_completed, created_at",
        )
